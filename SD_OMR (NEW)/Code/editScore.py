...
def editScore(path):
    ...
    fileName = "Edited_Graded_OMR" + time.strftime("%Y%m%d-%H%M%S", time.localtime()) + ".png"
    gradedOMRFileName = "Edited/" + fileName
    cv2.imshow("Final Image", imgFinal)                     # Display finalized image
    cv2.imwrite(gradedOMRFileName, imgFinal)
    cv2.waitKey(0)
    return fileName
